...
<template>
  <div class="space-y-6 bg-deep-radial pb-12">
    ...
    <template v-if="!showSearchMode">
      <!-- Sticky Category chips (sleek, rounded, lower height) -->
      <div 
        v-if="visibleCategories.length > 0"
        ref="chipsRow" 
        class="sticky top-0 z-[90] bg-[#050505]/95 border-b border-brand-border/40 py-2 -mx-4 px-4 overflow-x-auto whitespace-nowrap flex gap-1.5 scrollbar-hide"
      >
        <button
          ...
          class="px-3 py-1 rounded-full text-[10px] font-display font-black uppercase tracking-wider transition-editorial border select-none button-press"
      ...
      <div class="space-y-8 px-4">
        ...
        >
          <div :id="`category-${cat.id}`" class="pt-6 pb-2 flex flex-col gap-1.5">
            <h2 class="text-[11px] font-display font-black uppercase tracking-[0.25em] text-white">
              {{ cat.name }}
            </h2>
    ...
    <template v-else>
      <section class="space-y-4 px-4">
        <div class="flex items-center justify-between border-b border-brand-border/40 pb-3">
          <h2 class="text-[11px] font-display font-black uppercase tracking-[0.2em] text-brand-gray flex items-center gap-2">
          ...
          <span class="text-[9px] font-display font-black tracking-widest text-brand-gray uppercase">{{ searchResults.length }} items</span>
        </div>
